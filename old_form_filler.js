...
// upload file
export async function uploadDesktopDropzoneFile(fileUrl) {
  try {
    const input = document.querySelector("#desktop-drop");
    if (!input) {
      console.error("❌ Could not find input with ID #desktop-drop");
      return;
    }

    // Fetch the file from a URL (you can modify this to use Blob/File APIs instead)
    const response = await fetch(fileUrl);
    const blob = await response.blob();
    const filename = fileUrl.split("/").pop();
    const file = new File([blob], filename, { type: blob.type });

    // Attach file to input via DataTransfer
