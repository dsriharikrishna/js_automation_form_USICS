    ...
    (async () => {
      if (window.location.pathname.includes("accounts/paralegal/file-form")) {
        ...
        await formFiller.selectDropdownValueForCompany(
          "client",
          message.clientValue || "Company - Signitives Technologies LLC",
          "id"
        );
        await formFiller.delay(5000);
      }
      ...
      if (
        ...
        await formFiller.delay(20000);
        // Radio: Exempt status
        const isExempt = rowData["PAID HIGHER"] === "Yes" ? true : false;
        await formFiller.selectRadioOption(
      ...
      if (
        ...
        await formFiller.delay(20000);
        const radioValue = clientInfo["Is Nonprofit or Higher Edu"] || "false";
      ...
      if (window.location.pathname.includes("employment/work-location")) {
        await formFiller.delay(5000);
        console.log("⚙️ Work Location autofill started...");
        ...
        if (true) {
          ...
          const workLocationFields = [
            ...
            {
              nameOrId:
                "employment.workLocation.additionalWorkAddresses.0.city",
            ...
            {
              nameOrId:
                "employment.workLocation.additionalWorkAddresses.0.state",
              value:
          ...
          await formFiller.delay(6000);
          await formFiller.selectMuiAutocomplete(
            "employment.workLocation.additionalWorkAddresses.0.state",
            rowData["WORK STATE"] || "",
            "id"
          );
          ...
          if (response === true) {
            for (let i = 0; i < 5; i++) {
              ...
              const submitted = await formFiller.saveButton(
                "table-submit-button"
              );
              if (submitted) {
              ...
              console.warn(`⏳ Retry ${i + 1}: Waiting to re-attempt submit`);
              await formFiller.delay(2000);
            }
            console.log("Work Location fields filled successfully");
          } else {
            setTimeout(() => {
              formFiller.saveButton("table-submit-button");
              formFiller.clickButtonById("button-button");
            }, 5000);
            console.warn("❌ Failed to fill Timeout Work Location fields");
          }
        }
        await formFiller.delay(5000);
        await formFiller.breakValidationAndSubmit();
